...
def annotate(ann,ccols,ocols,clust,c):
    ...
    for line in to_add:
        ...
        #else:
            #qmap = sns.color_palette(maps[k], n_colors=(j+1))
            #cur = sns.heatmap(df, cmap=maps[k], n_colors=(j+1), rasterized=True)
        new.set_figheight(c)
        new.set_figwidth(2)
        new.set_dpi(600)
        #        plt.show(new)
...
if args['-c'] != None:
    ...
    res = sns.clustermap(df,method='weighted',cmap=usermap,metric='euclidean',figsize=(c, r),rasterized=True)
else:
    res = sns.clustermap(df,method='weighted',cmap='Blues',metric='euclidean',figsize=(c, r),rasterized=True)
