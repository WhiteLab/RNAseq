...
def report(sample,ref_gtf,tx_gtf):
    ...
    for line in rh:
        ...
        if tx_id not in ref:
            ...
            ref[tx_id]['name']=name
    rh.close()
    log(loc,date_time() + 'Completed loading annotation, parsing and annotating cufflinks output file ' + tx_gtf + '\n')
    rpt=open(sample + '.report.txt','w')
    rpt.write('chr\tstrand\tstart\tend\tscore\tgeneid\ttxid\tgene_sym\tgene_biotype\tfpkm\tconflo\tconfhi\tcoverage\tread_support\n')
    th=open(tx_gtf,'r')
...
if __name__ == "__main__":
    import argparse
    parser=argparse.ArgumentParser(description='Transcript annotation summart report.  Parses cufflinks trnascripts.gtf and adds gene symbols and transcript biotype')
    parser.add_argument('-sa','--sample',action='store',dest='sample',help='Sample/location name prefix')
    parser.add_argument('-r','--reference',action='store',dest='ref_gtf',help='Reference gtf file used to annotate - preferably GENCODE with gene name and gene type')
